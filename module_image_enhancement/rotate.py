...
from PIL import Image as PILImage  # Rename the PIL Image class
...
def correct_image_orientation(image):
    model_location = 'weight/model.h5'
    model = load_model(model_location)
    
    img_gray = image.convert('L')
    img_arr = np.array(img_gray)
    
    rotated_img = correct_image(model, img_arr)
    
    if rotated_img is None:
        print("Corrected image orientation returned None")
        return None
...
def correct_image(model, img, threshold=0.6):
    ...
    predicted_angle = np.argmax(model.predict(img_arr)) - 45
    print("Predicted angle:", predicted_angle)
    
    rotated_img = rotate(np.array(img), -predicted_angle)
    new_image = PILImage.fromarray(rotated_img)
