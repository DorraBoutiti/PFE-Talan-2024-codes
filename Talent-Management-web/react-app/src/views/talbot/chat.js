...
const ChatbotInterface = () => {
    ...
    const handleSendMessage = async () => {
        if (inputValue.trim() !== '') {
            ...
            try {
                const response = await fetch('http://localhost:4000/chatbot/message', {
                    method: 'POST',
                    headers: {
    ...
    return (
        <Box sx={{ backgroundColor: '#f4f6f8', minHeight: '0vh', display: 'flex', justifyContent: 'center', alignItems: 'center', padding: 0 }}>
            <Card sx={{ maxWidth: 1000, width: '100%', backgroundColor: '#ffffff', boxShadow: 3, borderRadius: 2 }}>
                <CardContent>
                    <Typography variant="h5" gutterBottom sx={{ color: '#2196f3', display: 'flex', alignItems: 'center' }}>
                        <ChatIcon sx={{ marginRight: 1 }} /> TALBOT
                    </Typography>
                    <Box
